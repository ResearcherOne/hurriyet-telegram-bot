...
let _adminIdList = [160010038, 262637366]; //262637366
...
var initializeCommandHandlers = function() {
	...
	app.command('send_help_to', (ctx) => {
		const userId = ctx.message.from.id;
		if (_isSysAdmin(userId)) {
			const commandString = lowerCase(ctx.message.text, 'tr');
			let [cmdName, helpNeededId] = commandString.split(' ');
			ctx.reply("Help is sent to "+ helpNeededId);
			_sendMessageToUser(parseInt(helpNeededId),"Görünüşe göre hiçbir kelimeyi takibe almamışsın. İlgilendiğin kelimeyi içeren haber bildirimlerini almak için '/bildirim kelime' komutunu kullanabilirsin.");
		}
	});
