...
var initializeCommandHandlers = function() {
	app.hears('perish-news-cache', (ctx) => {
		...
		if (_isSysAdmin(userId)) {
			...
			if (a && b)
				ctx.reply("Hey admin! newsDataCollection and newsIdCollection successfull perished.");
			else
	...
	app.hears('toggle-monitor-mode', (ctx) => {
		const userId = ctx.message.from.id;
		if (_isSysAdmin(userId)) {
	...
	app.hears('fetch-latest-news', (ctx) => {
		const userId = ctx.message.from.id;
		if (_isSysAdmin(userId)) {
